...
class SetBaseImageUseCase(
    ...
    suspend operator fun invoke(baseImage: Bitmap) {
        Timber.tag("SetBaseImageUseCase").d("Invoking SetBaseImageUseCase.")
        try {
            repository.setBaseImage(baseImage)
            Timber.tag("SetBaseImageUseCase").d("Base image set successfully.")
        } catch (e: Exception) {
            Timber.tag("SetBaseImageUseCase").e(e, "Error in SetBaseImageUseCase.")
            throw e
        }
    }
}
